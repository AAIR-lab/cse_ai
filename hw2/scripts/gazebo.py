...
class GazeboRunner:
    ...
    def setup_gazebo(self, subjects, books, seed):
        ...
        # Generate the maze.
        generate_maze(subjects, books, seed)
...
def run_gazebo_simulation(line_no, line):
    ...
    subjects = int(line[0].strip())
    books = int(line[1].strip())
    ...
    print("[BEGIN] Running Gazebo simulation on line_no=%u with subjects=%u"
        ", books=%u, seed=%u" % (line_no, subjects, books, seed))
    ...
    gazebo_runner.setup_gazebo(subjects, books, seed)
    gazebo_runner.run_gazebo(actions)
    gazebo_runner.cleanup_gazebo()
    
    print("[  END] Running Gazebo simulation on line_no=%u with subjects=%u"
        ", books=%u, seed=%u" % (line_no, subjects, books, seed))
...
if __name__ == "__main__":
    ...
    if args.line is None:
        ...
        run_gazebo_simulation(line_no, last_line)
